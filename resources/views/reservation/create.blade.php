...
@section('content')
        ...
        <div class="flex justify-between md:flex-row flex-col ">
            ...
            <div class="md:w-2/3  md:border-r border-gray-800 p-2">
                ...
                <div class=" flex items-end mt-8 ms-4">
                    ...
                    <p>
                        <span
                            class=" text-3xl font-bold text-gray-800 ms-3 me-1 border border-gray-800 p-2 rounded-md">₱{{ $car->price_per_day }}
                            </span>
                    </p>
                </div>
                ...
                <div class="px-6 md:me-8">
                    <form id="reservation_form" action="{{ route('car.reservationStore', ['car' => $car->id]) }}"
                        ...
                        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                         ...
                         <!-- Payment method -->
        <div class="mt-10">
            <label for="payment_method" class="block text-sm font-medium leading-6 text-gray-900">Select Payment Method</label>
            <div class="mt-2">
                <select name="payment_method" id="payment_method" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-pr-400 focus:ring focus:ring-pr-400 focus:ring-opacity-50" required>
                ...
                <div id="card_holder_name_field" class="mt-6 hidden">
            <label for="card_holder_name" class="block text-sm font-medium leading-6 text-gray-900">Card Holder Name</label>
            <div class="mt-2">
                <input type="text" name="card_holder_name" id="card_holder_name" placeholder="Name" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-pr-400 focus:ring focus:ring-pr-400 focus:ring-opacity-50" required>
            </div>
        </div>

        <!-- Card number -->
        <div id="card_number_field" class="mt-6 hidden">
     ...
     <div class="mt-2">
         ...
         placeholder="xxxx xxxx xxxx xxxx" required>
                    ...
                    <div class="mt-12 md:block hidden  ">
                            <button type="submit"
                                class="text-white bg-black p-3 w-full rounded-lg font-bold hover:bg-pr-400 shadow-xl hover:shadow-none ">Reserve
                                Now</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="md:w-1/3 flex flex-col justify-start items-center">
                <div class="relative mx-3 mt-3 flex h-[200px] w-3/4   overflow-hidden rounded-xl shadow-lg">
                    ...
                    <span
                        class="absolute w-24 h-8 py-1 top-0 left-0 m-2 rounded-full bg-gray-800 px-2 text-center text-sm font-medium text-white">{{ $car->reduce }}
                        %
                        OFF</span>
                </div>
                ...
                <div class=" w-full   mt-8 ms-8">
                    <p id="duration" class="font-car text-gray-600 text-lg ms-2">Duration: <span
                            class="mx-2 font-car text-md font-medium text-gray-700 border border-gray-800 p-2 rounded-md "> X
                            days</span>
                    </p>
                </div>
                ...
                <div class=" w-full   mt-8 ms-8">
                    <p id="total-price" class="font-car text-gray-600 text-lg ms-2">Total Price: <span
                            class="mx-2 font-car text-md font-medium text-gray-700 border border-gray-800 p-2 rounded-md ">₱ 0.00
                            </span>
                    </p>
                </div>
    ...
    <script>
    // Show/hide card holder name and card number fields based on payment method selection
    document.getElementById('payment_method').addEventListener('change', function() {
        var cardHolderNameField = document.getElementById('card_holder_name_field');
        var cardNumberField = document.getElementById('card_number_field');

        if (this.value === 'select') {
            cardHolderNameField.classList.add('hidden');
            cardNumberField.classList.add('hidden');
        } else if (this.value === 'debit_card' || this.value === 'credit_card') {
            cardHolderNameField.classList.remove('hidden');
